...
async function procesarCronica(paso, mensaje, datos, telefono, nombreWhatsApp) {
  ...
  if (resultado.nivel === 3) {
    ...
    await crearNotificacion('urgente', `🚨 Alerta grave — ${enfDef.nombre}`, `${nombreWhatsApp} reportó valores fuera de rango en su seguimiento crónico. Requiere agendar consulta de seguimiento con prioridad.`, datos.paciente_id, null, { origen: 'seguimiento', categoria: 'grave', etiqueta: 'CRÓNICO', estado_validacion: 'pendiente' });
    ...
    await crearNotificacion('urgente', `⚠️ Valores de atención — ${enfDef.nombre}`, `${nombreWhatsApp} reportó valores de atención en su seguimiento crónico. Considere agendar una consulta de seguimiento.`, datos.paciente_id, null, { origen: 'seguimiento', categoria: 'medio', etiqueta: 'CRÓNICO', estado_validacion: 'pendiente' });
  } else {
